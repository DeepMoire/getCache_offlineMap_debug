// ⚠️ these tests use a real live-Worker manifest as fixture — the phone once had NO CODE for a `png/` key, so the picture arrived, stored, and never rendered.
...
describe("the phone receives the worker's picture", () => {
	...
	it("⛔ phone and worker agree on the key spelling", () => {
		// ⚠️ a key is a lookup — a formatting difference is a total miss with no error anywhere.
	...
	it("a vector-tile pack is NOT mistaken for a picture", () => {
		// ⚠️ the old format must keep working through the switchover — otherwise the fix breaks every already-stored area.
	...
	it("⛔ a picture with NO box is refused, never guessed", () => {
		// The previous generation of this bug GUESSED the tile's box and drew roads 89 km from the pin (measured at Timbuktu) — no box → no picture.
		expect(
	...
	it("⛔ image corners are [NW, NE, SE, SW] — a swap mirrors silently", () => {
		// ⚠️ MapLibre does not validate this order — wrong order renders a mirrored or rotated picture with no error at all.
	...
	it("⛔ THE BOX IS DERIVABLE FROM THE KEY — 0 m against the LIVE worker", () => {
		// ⚠️ if this ever goes red, the halves have drifted — reconcile them, do not start storing the box to paper over it (two copies of one derivable fact is how the 45 km offset happened).
