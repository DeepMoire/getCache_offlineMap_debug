 ...
 *
 * This is the test that makes the rapper move safe. The engine is meant to run on
 * a page that has no TinyBase, no mapStore, no Supabase and no Capacitor — a
 * contractor's `/offline/debug`. Nothing else checks that: the app's own tests all
