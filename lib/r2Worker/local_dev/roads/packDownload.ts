...
// · 45 = THE PACK LAYERS CONTRACT (lib/contract/packLayers.ts). The blob carries
// roads + water (lake/pond polygons, river/canal lines) + places (city…hamlet,
// matched on `kind_detail` — every v4 places feature is `kind:locality`, so the
// old `kind` allowlist kept none) + pois (hospital, camp_site). Every pv<=44
// blob on a device is roads-only (MEASURED: one source layer); this bump is
// what makes it re-download.
