...
describe("THE BLOB — one radius, every zoom", () => {
	...
	it("the Worker's spec matches the client's, exactly", () => {
		...
		// The Worker lives INSIDE this child now (worker/src/, moved 28 Aug 2026
		// from ReTreever/workers/offline-tiles), so the climb is four levels from
		// this file and names no parent. Its grid.ts is a one-line re-export of
		// lib/contract/grid.ts — ONE definition, not two kept in step — so the
		// zoom is read through that re-export rather than from a scraped copy.
