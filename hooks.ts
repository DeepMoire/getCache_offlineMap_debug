 ...
 *
 * This child serves /offline and /demo, so "/" has nothing of its own to be. Mounted
 ...
 * without a load running at all, so "/offline" stays the only url that names
...
export const reroute: Reroute = ({ url }) => {
	if (url.pathname === "/") return "/offline";
};
