...
// ── layer keep-sets (must match the client decode's roads+water-below-DETAIL_INNER_Z
// rule — decodeBucketKind in v4Decode.ts) ──
const ROADS_ONLY = new Set(["roads"]); // corridor packs: a thin route ribbon, no base
// 25 km z12 OUTER ring: roads + places ONLY. `earth` dropped (its coarse z12 fill is
// ugly tile-square blocks); `places` (tiny town points) gives major-town labels.
const OUTER_RING = new Set(["roads", "places"]);
// 5 km z15 INNER ring: the MINIMUM legible basemap — roads + WATER (the small detailed
// lake right here) + a couple of POI icons. Water kept HERE but NOT on the outer ring.
// `landuse` carries the forest / wetland / field FILLS — the thing that makes this
// read as land rather than a road diagram. Shipped so the phone never has to
// MANUFACTURE them: the on-device decoder existed almost entirely to synthesise the
// `land` source-layer (and sample `roadlabels`) out of a merged GeoJSON graph, and
// that round trip MEASURED 622 MB climbing 155 MB/s on a wide z9 camera.
//
// NOT `landcover`. The old on-device decode read both, so the obvious move was to
// ship both — but MEASURED against this archive, `landcover` is EMPTY at every zoom
// and every site sampled (home / Cascades / Vancouver, z12 / z13 / z15: 0 features
// each time) while `landuse` carries all of it, including the `wood`, `wetland`,
// `grass`, `meadow` and `national_park` kinds the six v4-land-* style layers filter
// on. Listing an empty layer costs nothing but implies a dependency that isn't real.
// ⛔ ROADS ONLY — water and landuse REMOVED 2026-08-18, and both for the same
// reason: BUILD SPEED. A cold 30 km blob is ~3,950 z15 tiles and water polygons
// are the heaviest thing in them; the build was taking 56 s, which timed the
// client out and made the whole feature look broken.
//
// The user's call on seeing the first working blob: land cover — "get rid of the
// green ... let's just do the roads for now, it's not meant to be a satellite
// image"; then water — "if it's that painfully slow to bring in the water it's
// not really worth very much ... we've got to get feedback faster".
//
// `landuse` was ALREADY dead weight: the six v4-land-* style layers that read it
// were deleted client-side, so those bytes shipped and painted nothing.
//
// `pois` stays — hospital + camp_site icons are a few features per tile.
//
// ⚠️ Bring water back only WITH a measurement. It previously came from a 5 km
// radius, not the full 30, so restoring it at 30 km is NOT the old cost.
const INNER_RING = new Set(["roads", "pois"]);
// 25 km z13 MID ring: the band the DEFAULT CAMERA sits in. ROADS ONLY.
//
// Water was tried here and MEASURED at 655 kB vs 285 kB on the sparse test site — the
// mid ring alone became 77% of the whole pack, for lake polygons the z15 core already
// draws where the user is standing. `places` (town labels) is left to the z12 outer
// ring, which already draws them across the same ground; duplicating them here would
// double-render every label. This ring exists to give z13-z14 a road network to
// stretch, nothing more.
...
/** Which layers survive in a tile at zoom `z`. Exported so the ring routing is
 *  testable — three rings routed by an inline nested ternary is exactly the shape
 *  that regresses silently (the z13 ring initially fell through to OUTER_RING and
 *  lost all its water). */
export function keepSetForZoom(z: number, corridor: boolean): ReadonlySet<string> {
  if (corridor) return ROADS_ONLY;
  // The DETAIL level carries the full basemap (roads + water + landuse + pois).
  // Every shallower level is roads + places only — at those zooms the whole disc
  // is a few pixels wide, so water polygons and landuse fills are invisible
  // weight. ONE threshold, from the spec; no per-ring special cases.
  //
  // ⛔ THE THRESHOLD IS THE LEVEL THE PACK READS. This compared against
  // `BLOB_DETAIL_Z` (15) while `buildPack` reads at `BLOB_DETAIL_LEVEL` (13),
  // so `13 >= 15` was false and EVERY tile fell through to OUTER_RING — which
  // is precisely the regression the docstring above already warns about
  // ("the z13 ring initially fell through to OUTER_RING and lost all its
  // water"). It dropped `pois` from every pack built anywhere, and `pois` is
  // the ENTIRE offline hospitals feature (mvtFilter keeps hospital +
  // camp_site from it). Derived, so it cannot drift again.
    ...
    while (next < disc.length) {
      ...
      try {
        ...
        if (t?.data?.byteLength) {
          // Corridor → roads-only. Outer ring (below the inner detail zoom) → roads +
          // places. Inner z15 → roads + water + pois. Each kept layer is ALSO
          // kind-filtered to the rendered kinds (mvtFilter).
